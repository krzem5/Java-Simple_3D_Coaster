	...
	public static class Main_ extends Constants implements GLEventListener{
		...
		public Main_(String[] args){
			...
			if (args.length==1){
				...
				IO.load_from_file(this.game,args[0]);
			}
			this.frame_init();
			this.run();
		}
		...
		public void init(){
			...
			this.game.ll.add(new Light(new Vector3(0,65,0)));
		}
		...
		public void frame_init(){
			Main.Main_ cls=this;
			this.frame=new JFrame("Simple 3D Coaster");
		...
		public void draw(GL2 gl){
			...
			this.cam.draw(gl);
			// gl.glLightfv(GL2.GL_LIGHT0,GL2.GL_POSITION,new float[]{0,10000,0,1},0);
			// gl.glLightfv(GL2.GL_LIGHT0,GL2.GL_DIFFUSE,new float[]{0.1f,0.1f,0.1f,1f},0);
			// gl.glLightfv(GL2.GL_LIGHT0,GL2.GL_SPECULAR,new float[]{0f,0f,0f,0f},0);
			// gl.glLightfv(GL2.GL_LIGHT0,GL2.GL_SPOT_EXPONENT,new float[]{128f},0);
			this.game.draw(gl);
			gl.glFlush();
		}
