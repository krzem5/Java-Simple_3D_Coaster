...
public class Train extends Constants{
	...
	public void draw(GL2 gl){
		...
		int CAR_COUNT=3;
		double CAR_SEPARATION_DIST=1.75;
		for (int i=0;i<CAR_COUNT;i++){
			this._draw_wheel_pair(gl,this.doff-i*CAR_SEPARATION_DIST,i>1,i<CAR_COUNT-1);
		}
		gl.glEnd();
	}
	...
	private void _draw_wheels(GL2 gl,double doff){
		...
		double foff=this._lerp(this.tl.tdt.tsl[fidx],this.tl.tdt.tsl[fidx+1],ft);
		...
		double boff=this._lerp(this.tl.tdt.tsl[bidx],this.tl.tdt.tsl[bidx+1],bt);
