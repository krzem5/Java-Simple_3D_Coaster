...
public class TrackRenderer extends Constants{
	...
	public void draw(GL2 gl){
		...
		this._track_slice(gl,this.cv.get(1).sub(this.cv.get(0)),this.cv.get(2).sub(this.cv.get(1)).center(this.cv.get(1).sub(this.cv.get(0))),this.cv.get(0),this.cv.get(1),(double)this.rt.get(0),(double)this.rt.get(1),false);
		for (int i=1;i<this.cv.size()-2;i++){
			this._track_slice(gl,this.cv.get(i+1).sub(this.cv.get(i)).center(this.cv.get(i).sub(this.cv.get(i-1))),this.cv.get(i+2).sub(this.cv.get(i+1)).center(this.cv.get(i+1).sub(this.cv.get(i))),this.cv.get(i),this.cv.get(i+1),(double)this.rt.get(i),(double)this.rt.get(i+1),false);
		}
		this._track_slice(gl,this.cv.get(this.cv.size()-1).sub(this.cv.get(this.cv.size()-2)).center(this.cv.get(this.cv.size()-2).sub(this.cv.get(this.cv.size()-3))),this.cv.get(this.cv.size()-1).sub(this.cv.get(this.cv.size()-2)),this.cv.get(this.cv.size()-2),this.cv.get(this.cv.size()-1),(double)this.rt.get(this.rt.size()-2),(double)this.rt.get(this.rt.size()-1),true);
		gl.glColor3d(this.tl.tcb[0],this.tl.tcb[1],this.tl.tcb[2]);
	...
	private void _track_slice(GL2 gl,Vector3 s,Vector3 e,Vector3 so,Vector3 eo,double soff,double eoff,boolean end){
		Vector3 sa=s.normalize();
		// Vector saa=sa.angle();
		// Vecotr3 sp=Vector3.sphere(TRACK_PIECE_BIGGER_RADIUS,saa.x-Math.PI/2,saa.y).rotate(sa,soff);
		Vector3 sp=sa.plane_perpendicular().mag(TRACK_PIECE_BIGGER_RADIUS).rotate(sa,eoff);
